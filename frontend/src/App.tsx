...
type Pollutant = { name: string; value: number }
...
function App(): JSX.Element {
  ...
  const fetchData = async (isRefresh = false) => {
    try {
      ...
      if (data.aqi > 100) {
        setNotification('Air quality is unhealthy. Avoid outdoor exposure.')
      }
    } catch (err) {
      console.error(err)
      setError(
          ...
          <div className="w-full flex flex-col gap-8">
            ...
            <section className="bg-white rounded-2xl shadow-sm p-6 flex flex-col md:flex-row items-center justify-between gap-6">
              <div>
                ...
                <p className="text-gray-500 text-sm">Last updated: {aqiData.timestamp}</p>
              </div>
              <div className="text-center">
                ...
                <p className="text-sm text-gray-600">{getAQIStatus(aqiData.aqi)}</p>
              </div>
            </section>
            ...
            <section className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {aqiData.pollutants.map((item) => (
                <div key={item.name} className="bg-white rounded-xl shadow-sm p-4 text-center">
            ...
            <section className="bg-white rounded-2xl shadow-sm p-6">
              <h3 className="font-medium mb-4 text-gray-700">Forecast (next 24h)</h3>
            ...
            <section className="bg-white rounded-2xl shadow-sm p-6">
              ...
              <p className="text-sm text-gray-600 leading-relaxed">{aqiData.health_advice}</p>
            </section>
          </div>
        )}
      </main>
